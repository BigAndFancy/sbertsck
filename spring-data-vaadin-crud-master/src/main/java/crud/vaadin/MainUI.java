...
public class MainUI extends UI {
    ...
    private MGrid<Departments> list = new MGrid<>(Departments.class)
    ...
    public MainUI(DepartmentsRepo r, DepartmentForm f, EventBus.UIEventBus b) {
        this.repo = r;
        this.departmentForm = f;
        this.eventBus = b;
    }
        ...
        list.setDataProvider(
                ...
                (sortOrder, offset, limit) -> {
                    ...
                    if (startPage != endPage) {
                        List<Departments> page0 = repo.findByNameLikeIgnoreCase(likeFilter, PageRequest.of(startPage, pageSize, sortDirection, sortProperty));
                        page0 = page0.subList(offset % pageSize, page0.size());
                        List<Departments> page1 = repo.findByNameLikeIgnoreCase(likeFilter, PageRequest.of(endPage, pageSize, sortDirection, sortProperty));
                        page1 = page1.subList(0, limit - page0.size());
                        List<Departments> result = new ArrayList<>(page0);
                        result.addAll(page1);
    ...
    protected void edit(final Departments phoneBookEntry) {
        departmentForm.setEntity(phoneBookEntry);
        departmentForm.openInModalPopup();
    }

    @EventBusListenerMethod(scope = EventScope.UI)
    public void onPersonModified(DepartmentModifiedEvent event) {
        listEntities();
        departmentForm.closePopup();
    }
